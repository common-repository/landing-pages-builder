    ...
    class WishpondShortcode {
      ...
      public static function wpsc($attrs) {
        ...
        if (empty($attrs['id']) && empty($attrs['merchant_id'])) {
          return "<h1>" . __('wpsc shortcode needs either id or merchant_id attribute', 'landing-pages-builder') . "</h1>";
        }
      ...
      public static function wp_campaign($attrs) {
        ...
        if (empty($attrs['campaign_id']) || empty($attrs['merchant_id']) || empty($attrs['write_key'])) {
          return "<h2 style='color:red;'>" . __('wp_campaign shortcode needs campaign_id, merchant_id, and write_key attributes', 'landing-pages-builder') . "</h2>";
        }
