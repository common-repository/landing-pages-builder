    ...
    class WishpondPlugin {
        ...
        if (isset($_POST['submit']) && $_POST['submit']) {
          if (isset($_POST['token'])) {
            ...
            if ($token === get_option('wishpond_auth_token')) {
              $error_message = __('Token value is the same', 'landing-pages-builder') . ': ' . $token;
            } else {
              $saved = update_option('wishpond_auth_token', $token, '', 'no');

              if ($saved) {
                $notice_message = __('Successfully updated token', 'landing-pages-builder') . ': ' . $token;
              } else {
                $error_message = __('Unable to update token', 'landing-pages-builder') . ': ' . $token;
              }
            }
          }
        }
      ...
      public function publish_campaign($data) {
        if (!is_user_logged_in()) {
          wp_die(__('Not logged in', 'landing-pages-builder'));
        }

        if (!current_user_can('activate_plugins')) {
          wp_die(__('Not enough permissions', 'landing-pages-builder'));
        }

        if (!WishpondUtilities::permalink_structure_valid()) {
          $message = __('Invalid permalink structure. Please go to "Settings"->"Permalinks" and make sure your permalinks use the "postname"', 'landing-pages-builder');
          ...
          $action   = __('saved', 'landing-pages-builder');

          if ($campaign->post_id !== -1) {
            $saved  = $campaign->update($path);
            $action = __('updated', 'landing-pages-builder');
          } else {
            $saved = $campaign->save();
          }
          ...
          if ($saved) {
            ...
            $message = WishpondUtilities::json_message('updated', __('Campaign ', 'landing-pages-builder') . $action . __(' successfully. ', 'landing-pages-builder') . $campaign_url);
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'landing-pages-builder'));
          }
        }
      ...
      public function delete_campaign($data) {
        ...
        if ($campaign) {
          if ($campaign->remove()) {
            $message = WishpondUtilities::json_message('updated', __('Campaign removed successfully.', 'landing-pages-builder'));
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'landing-pages-builder'));
          }
        } else {
          $message = WishpondUtilities::json_message('error', __('Campaign not found.', 'landing-pages-builder'));
        }
      ...
      function wishpond_load_plugin_textdomain() {
        load_plugin_textdomain( 'landing-pages-builder', FALSE, basename( dirname( __FILE__ ) ) . '/lang/' );
      }
    }
  }
